...
| D-33 | The runtime boundary is a tiny deterministic API (`applyCommand`, `getLegalActions`, `projectState`, `hashState`, ...) with injected RNG and no clock/network. See `shiba-sdk/docs/engine-api.md`. | Proposal | Not yet reviewed. Where the runner lives is open (O-14). |
...
| O-14 | Where does the rules code shared by the app and the Elixir server live, and where does the runner that plays it live? The name `shiba-core` is unused and was meant for shared rules code. | Options: a `shiba-core` repo of rules code later; the file `sht build-policy` produces, copied into each app; runner inside `shiba-app` and `shiba-mps`. Decide when a real game's rules exist. |
| O-12 | Confirm QuickBEAM as the JS host in `shiba-mps` (leading candidate) versus a Node worker pool or WASM. Test engine parity, limits and latency in a spike. | Direction set by D-30. See `shiba-mps/docs/scope.md`. Progress: QuickBEAM loads a real bundle and reads its contract (D-39). Tested on toy policies: 35 of 38 engine checks match V8 (the differences are harmless; locale APIs are banned by D-40), limits and isolation held. On a heavy toy policy (324 KB bundle, 4,000 objects) one round of rules takes about 1.4 ms per 1,000 items, about 3 to 4 times slower than V8 and well inside budget, and each running match uses about 3 MB. Watch out: QuickBEAM silently truncates results over 10,000 values unless the limit is raised (it is, and tested). Not yet tested: a long soak, and a real game's rules. Details in `shiba-mps/docs/BACKLOG.md` (`MP-0001`). |

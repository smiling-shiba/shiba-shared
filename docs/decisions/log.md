...
| D-45 | A mod is templates and assets only, never policy code. One active policy; many mods may layer templates/assets on top of it, later wins on a same-kind-same-id collision. A ruleset needing mechanics the active policy's functions and hooks do not expose is not a mod — it is a different pack, sharing nothing with the base pack, built and published on its own. | Decided | Matches how modding actually plays out in practice (Minecraft, Factorio): the vast majority of mods are content, not new engine code, and the systems that scaled best keep data freely stackable while code never merges. A mod can never do more than a template already could, so it needs no new trust or execution model — D-38's whole-pack lock/sign/verify applies unchanged, a mod is just a pack-shaped folder without `policies/`. Flexibility for modders comes from how generous the policy author is with functions and hooks, a call made when writing the policy, not from the pack format. Answers `SH-0009`, and gaps 2-3 and 5-6 in `design/packs-and-modding.md`. |

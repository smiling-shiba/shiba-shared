...
| `shiba-core` | SDK and deterministic runtime for policies. Generates a policy's contract. Ships a toy policy as its example and test fixture. | Public |
...
```text
shiba-core  <-  a game's policy        (written with the SDK)
policy contract  <-  shiba-tools       (reads the generated contract, never policy source)
shiba-core  <-  shiba-app              (runtime for previews and local play)
shiba-core  <-  shiba-mps              (policy bundle loaded into the BEAM; source maps optional, D-30)
```

...
| `shiba-mps` | Multiplayer server (Elixir/Phoenix): accounts, matchmaking, Channels/Presence, seasons, entitlements, rules registry. It loads the `shiba-sdk` bundle into the BEAM and is the source of truth for multiplayer outcomes (D-30). | Private |
...
built policy bundle  <-  shiba-mps   (loaded into the BEAM; source maps optional, D-30)
```
...
- **Keep the shared surface small.** Where shared runner code and command/event types live is open (O-14). Input abstractions stay in the client.
